...
const Video = () => {
  ...
  const handleDelete = async (deleteEndpoint) => {
    try {
      ...
      const response = await axios.delete(`${API_BASE_URL}${deleteEndpoint}/${courseName}/${moduleName}/${unitName}/${chapterName}/${videoName}`);
  ...
  useEffect(() => {
    if (resRef.current) {
      ...
      if (selectedChapter) {
        console.log("selected file : ", selectedChapter.contents.videos.map(note => note.name))
        setFilesOption(selectedChapter.contents.videos.map(note => note));
          ...
          <div className='form '>
            <form>
              ...
              <button
                ...
                onClick={() => handleDelete('/course/video')}
              >
                Delete File
              </button>
            </form>
