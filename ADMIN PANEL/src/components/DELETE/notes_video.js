...
const NotesVideoDelete = () => {
  ...
  const handleDelete = async (deleteEndpoint) => {
    try {
      ...
      const response = await axios.delete(`${API_BASE_URL}${deleteEndpoint}/${courseName}/${moduleName}/${unitName}/${chapterName}/${fileName}`);
  ...
  useEffect(() => {
    if (resRef.current) {
      ...
      if (selectedChapter) {
        console.log("selected file : ", selectedChapter.contents.notes.map(note => note))
        setFilesOption(selectedChapter.contents.notes.map(note => note));
      }
    }
  }, [selectedChapterName])
          ...
          <div className='form '>
            <form>
              ...
              <button
                ...
                onClick={() => handleDelete('/course/file')}
              >
                Delete File
              </button>
            </form>
...
export default NotesVideoDelete;
